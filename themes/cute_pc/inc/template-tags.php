...
function _s_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
        ...
        foreach($cates as $cate) {
        	$list[] = '<a href="' . get_category_link( $cate->term_id ) . '" title="' 
                    . esc_attr( sprintf( __( "%sの投稿をすべて見る" ), $cate->name ) ) 
					. '">' . $cate->cat_name . '</a>';
        }
        
		$categories_list = implode(', ', $list);

        if ( $categories_list && _s_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Category: %1$s', '_s' ) . '</span>'."\n", $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', '_s' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged: %1$s', '_s' ) . '</span>'."\n", $tags_list ); // WPCS: XSS OK.
		}
	}
